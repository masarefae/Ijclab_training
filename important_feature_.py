...
def prepareInferenceData(data: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    ...
    input = data.drop(
     ...
     #save the name of columns that inter to model 
    ...
    # return x and y and array of column name
...
#index of the input  that choose to measurement it effect to the effectivity
index = 0
...
for clusteredEvent in  clusteredData:
    ...
    # Access and print the  column name
    column_name = x_test_df.columns[index]
    print(" column name:", column_name)
    
    for col_index in range(x_test.shape[0]):
         # save a fake value in the input that choose to measurement it is effect to the effectivity
         x_test[ col_index, index] = np.random.rand()
