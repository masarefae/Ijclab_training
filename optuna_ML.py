...
from ambiguity_solver_network3 import prepareDataSet, DuplicateClassifier, Normalise
...
def prepareInferenceData(data: pd.DataFrame) -> tuple[np.ndarray, np.ndarray]:
    ...
    column_names = input.columns.tolist()
...
index = 0
# Assuming other necessary imports and functions are defined above

# Prepare the CSV file for writing results
# ttbar events as test input
CKF_files = sorted(glob.glob("/data/atlas/callaire/Acts/ODD_data/odd_full_chain_03" + "/event000000000-tracks_ckf.csv"))
CKF_files2 = sorted(glob.glob("/data/atlas/callaire/Acts/ODD_data/odd_full_chain_01" + "/event0000000[0-9][0-9]-tracks_ckf.csv"))

data = readDataSet(CKF_files)
dataa = readDataSet(CKF_files2)
...
for event in dataa:
    clustered2 = clusterTracks(event)
    clusteredData2.append(clustered2)

t2 = time.time()

duplicateClassifier = torch.load("duplicateClassifier_withoutSeedDup(true)_algo1_3layer.pt")
...
for clusteredEvent in  clusteredData:
    # Prepare the data
    x_test, y_test, column_names = prepareInferenceData(clusteredEvent)
    #x_test2, y_test2 = prepareInferenceData(clusteredEvent2)
    # Write the network score to a list
    print ("before")
    print (x_test)
    
    
    # Access and print the first column name
    x_test_df = pd.DataFrame(x_test, columns=column_names)
    
    
    # Access and print the first column name
    column_name = x_test_df.columns[index]
    print(" column name:", column_name)
    
    for col_index in range(x_test.shape[index]):
        
         #print(f"pr value {x_test[col_index, 1]}   {col_index + 1}")
         x_test[ col_index, 0] = np.random.rand()
     
    print ("after")
    #print (x_test) 
    #x_test[0] = np.random.rand()
    #print (x_test)
...
duplicate_rate_initial = 100 * nb_duplicate / nb_track
fake_rate_initial = 100 * nb_fake / nb_track

    # Computed efficiencies
efficiency_good_track = 100 * nb_good_match / nb_part
efficiency_particle_reco = 100 * nb_reco_part / nb_part
duplicate_rate_computed = 100 * ((nb_good_match + nb_reco_duplicate) - nb_reco_part) / nb_reco_track
fake_rate_computed = 100 * nb_reco_fake / nb_reco_track

    # Computed speed
clustering_time_ms = (t2 - t1) * 1000 / len(CKF_files)
inference_time_ms = (t4 - t3) * 1000 / len(CKF_files)
total_time_ms = (end - start) * 1000 / len(CKF_files)

rate =100* (87.31031157717109 - efficiency_good_track) /87.31031157717109

with open('results_without'+  x_test_df.columns[index]+'_column.csv', mode='w', newline='') as file:
    writer = csv.writer(file)
    
    # Write header
    writer.writerow([
        "nb_particles", "nb_track", "duplicate_rate_initial", "fake_rate_initial",
        "nb_good_match", "nb_particle_reco", "nb_track_reco", "efficiency_good_track", 
        "efficiency_particle_reco", "duplicate_rate_computed", "fake_rate_computed",
        "clustering_time_ms", "inference_time_ms", "total_time_ms","rate"
    ])
    writer.writerow([
        nb_part, nb_track, duplicate_rate_initial, fake_rate_initial,
        nb_good_match, nb_reco_part, nb_reco_track, efficiency_good_track,
        efficiency_particle_reco, duplicate_rate_computed, fake_rate_computed,
        clustering_time_ms, inference_time_ms, total_time_ms,rate
    ])
