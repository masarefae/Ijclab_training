...
CKF_files = sorted(glob.glob("/data/atlas/callaire/Acts/ODD_data/odd_full_chain_03" + "/event00000000[0-9]-tracks_ckf.csv"))
...
for clusteredEvent in  clusteredData:
    ...
    #witch cloumn you want remove 
    remove_col = 1 
    for col_index in range(x_test.shape[0]):
        
         print(f"pr value {x_test[col_index, remove_col]}   {col_index + 1}") 
         x_test[ col_index, remove_col] = np.random.rand() 
...
with open('results_without_1_column.csv', mode='w', newline='') as file:
